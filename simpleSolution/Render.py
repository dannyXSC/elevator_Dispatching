...
#######################################################################
#######################################################################
def Building_Render(elevator_Number=default_Elevator_Number,
                    Layer_Number=max_Layer):
    pygame.init()
    pygame.font.init()
    ...
    screen_Width = elevator_Number * (
        ...
        (display_Elevator_Group_Elevator_Button_Hmargin +
         display_Elevator_Group_Elevator_Button_Width) * 3 +
        display_Elevator_Group_Elevator_Button_Group_Hmargin
    ) + display_Elevator_Group_Floor_Width + (
        display_Floor_Button_Width + display_Floor_Button_Hmargin) * 2 + 100
    screen_Height = Layer_Number * display_Elevator_Group_Elevator_Height
    ...
    ButtonStartX = display_Elevator_Group_Elevator_Width * elevator_Number + display_Elevator_Group_Floor_Width + (
        display_Floor_Button_Width + display_Floor_Button_Hmargin) * 2 + 50
    ...
    # 显示每个电梯的按钮的函数
    #######################################################################
    def showButton(content,
                   x,
                   y,
                   is_Active,
                   w=display_Elevator_Group_Elevator_Button_Width,
                   h=display_Elevator_Group_Elevator_Button_Height):
        myfont = pygame.font.SysFont(display_Elevator_Button_Font,
                                     display_Elevator_Button_Font_Size)
    ...
    def calFloorButtonPos():
        ...
        for i in range(Layer_Number):
            Ythis = screen_Height - (
                i + 1
            ) * display_Elevator_Group_Floor_Height + display_Elevator_Group_Floor_Height // 4
            retPos.append([[Xstart, Ythis],
                           [
    ...
    # run
    #######################################################################
    buildingHandle = Building(Layer_Number, elevator_Number)
    running = True
    while running:
        screen.fill(display_Elevator_Group_Bcolor)
        buildingHandle.step()
        ...
        show_Elevator_Group([[
            ...
            elevatorHandle.operation_Direction,
            elevatorHandle.status.remaining_Time
        ] for elevatorHandle in buildingHandle.elevators.list])
        showAllKindsOfButtonGroup([
            buildingHandle.elevators.list[i].get_Button_State()
            for i in range(elevator_Number)
        ])
        # TODO:
        showFloorButton([[floor.up_Button_State, floor.down_Button_State]
                         for floor in buildingHandle.layers])
        show_Floor()
        ...
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
                ...
                def ifClickButton(
                        ...
                        w=display_Elevator_Group_Elevator_Button_Width,
                        h=display_Elevator_Group_Elevator_Button_Height):
                    if cx >= bx and cx <= bx + w and cy >= by and cy <= by + h:
                        return True
                    else:
                        return False
                ...
                for i in range(elevator_Number):
                    ...
                    for j in range(Layer_Number):
                        if ifClickButton(cursorsPos[0], cursorsPos[1],
                                         allNButtonGroupPos[i][j][0],
                                         allNButtonGroupPos[i][j][1]):
                            buildingHandle.elevators.list[i].button_Click(j +
                                                                          1)
                            flag = 1
                            break
                    if flag == 0:
                        if ifClickButton(cursorsPos[0], cursorsPos[1],
                                         auxiliaryButtonGroupPos[i][0][0],
                                         auxiliaryButtonGroupPos[i][0][1]):
                            buildingHandle.elevators.list[i].close_Click()
                            flag = 1
                        elif ifClickButton(cursorsPos[0], cursorsPos[1],
                                           auxiliaryButtonGroupPos[i][1][0],
                                           auxiliaryButtonGroupPos[i][1][1]):
                            buildingHandle.elevators.list[i].open_Click()
                            flag = 1
                        elif ifClickButton(cursorsPos[0], cursorsPos[1],
                                           auxiliaryButtonGroupPos[i][1][0],
                                           auxiliaryButtonGroupPos[i][1][1]):
                            buildingHandle.elevators.list[i].alarm_Click()
                            flag = 1
                    if flag == 0:
                        # 监听楼层按钮
                        for i in range(Layer_Number):
